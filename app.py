...
def parse_question_to_sql(question: str) -> tuple:
    ...
    Returns (sql_query, params, description) tuple.
    """
    q = question.lower().strip()

    # --- Crisis queries (MUST be checked FIRST before 'negative/complaint') ---
    if any(w in q for w in ["crisis", "alert", "report", "summary", "brief"]):
        return (
            ...
            {},
            "Latest crisis reports (BART-generated summaries)"
        )
    ...
    if any(w in q for w in ["complaint", "negative", "worst", "bad", "problem", "issue"]):
    ...
    # --- User queries ---
    if any(w in q for w in ["user", "users", "active", "contributor"]):
        return (
            text("""
                SELECT u.user AS username, COUNT(*) AS total_reviews
                FROM fact_reviews f
    ...
    # --- Ratio / Percentage queries ---
    if any(w in q for w in ["ratio", "percentage", "percent", "split", "distribution"]):
        return (
            text("""
                SELECT
                    ROUND(SUM(CASE WHEN sentiment = 4 THEN 1 ELSE 0 END)::numeric / COUNT(*) * 100, 2) AS positive_pct,
                    ROUND(SUM(CASE WHEN sentiment = 0 THEN 1 ELSE 0 END)::numeric / COUNT(*) * 100, 2) AS negative_pct
                FROM fact_reviews
            """),
            {},
...
QUICK_QUESTIONS = [
    ...
    ("📉", "What is the sentiment percentage split?"),
    ("🚨", "Show me the latest crisis reports"),
    ("🏷️", "Who are the top mentioned entities?"),
    ...
    ("📝", "What are the top 10 complaints?"),
]
...
st.markdown("**🎯 Quick Questions** — click any button to get an instant answer:")

# Render buttons in a 4-column grid
btn_cols = st.columns(4)
for i, (icon, question) in enumerate(QUICK_QUESTIONS):
    with btn_cols[i % 4]:
        if st.button(f"{icon} {question}", key=f"qbtn_{i}", use_container_width=True):
...
# Determine which question to process (button click or typed)
user_question = None
...
# Also allow free-text typing (takes priority)
typed_question = st.chat_input("Type your own question... e.g. 'What are the top 3 complaints?'")
if typed_question:
